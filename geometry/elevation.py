...
class Elevation():
    ...
    def set_higher(self, other_E: Elevation) -> None:
        self.higher = other_E
    
    def set_lower(self, other_E: Elevation) -> None:
        self.lower = other_E
...
class ElevationMap():
    ...
    _ELEVATIONS: List[Elevation] = None
    _PATH_TO_ELEVATION_MAP = join(dirname(__file__), "assets/singapore-elevation.png")
    _IMG: Image = Image.open(_PATH_TO_ELEVATION_MAP)
    _ARR: np.array = np.array(_IMG.convert('RGB'))
    ...
    @staticmethod
    def _get_elevation_by_diff(
            result_e: Elevation, min_diff: float,
            r: float, g: float,
            b: float) -> float:
